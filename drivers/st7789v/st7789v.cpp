...
namespace LCD
{
    ...
    ST7789V::ST7789V(
        ...
        gpio_num_t rst,
        gpio_num_t bl)
        : dev{_dev}, _mosi{mosi}, _clk{clk}, _cs{cs}, _dc{dc}, _rst{rst}, _bl{bl} {};
    ...
    ST7789V::ST7789V(
        ...
        gpio_num_t rst,
        gpio_num_t bl)
        : dev{TFT_t{}}, _mosi{mosi}, _clk{clk}, _cs{cs}, _dc{dc}, _rst{rst}, _bl{bl} {};
    ...
    void ST7789V::spi_clock_speed(int speed)
    {
        ESP_LOGI(TAG, "SPI clock speed=%d MHz", speed / 1000000);
        clock_speed_hz = speed;
    }

    void ST7789V::spi_master_init(gpio_num_t GPIO_MOSI, gpio_num_t GPIO_SCLK, gpio_num_t GPIO_CS, gpio_num_t GPIO_DC, gpio_num_t GPIO_RESET, gpio_num_t GPIO_BL)
    {
        esp_err_t ret;
        ...
        ret = spi_bus_initialize(HOST_ID, &buscfg, SPI_DMA_CH2);
    ...
    bool ST7789V::spi_master_write_byte(spi_device_handle_t SPIHandle, const uint8_t *Data, size_t DataLength)
    {
    ...
    bool ST7789V::spi_master_write_command(uint8_t cmd)
    {
    ...
    bool ST7789V::spi_master_write_data_byte(uint8_t data)
    {
        static uint8_t Byte = 0;
        Byte = data;
    ...
    bool ST7789V::spi_master_write_data_word(uint16_t data)
    {
    ...
    bool ST7789V::spi_master_write_addr(uint16_t addr1, uint16_t addr2)
    {
    ...
    // Display OFF
    void ST7789V::DisplayOff()
    {
        spi_master_write_command(0x28); // Display off
    }

    // Display ON
    void ST7789V::DisplayOn()
    {
        spi_master_write_command(0x29); // Display on
    }
    ...
    void ST7789V::DrawArrow(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1, uint16_t w, uint16_t color)
    {
        ...
        DrawLine(x1, y1, L[0], L[1], color);
        DrawLine(x1, y1, R[0], R[1], color);
        DrawLine(L[0], L[1], R[0], R[1], color);
    }
    ...
    int ST7789V::DrawChar(FontxFile *fxs, uint16_t x, uint16_t y, uint8_t ascii, uint16_t color)
    {
        ...
        rc = Font::GetFontx(fxs, ascii, &pw, &ph);
        if (_DEBUG_)
    ...
    int ST7789V::DrawString(FontxFile *fx, uint16_t x, uint16_t y, uint8_t *ascii, uint16_t color)
    {
        ...
        for (int i = 0; i < length; i++)
        {
            ...
            if (dev._font_direction == 0)
                x = DrawChar(fx, x, y, ascii[i], color);
            if (dev._font_direction == 1)
                y = DrawChar(fx, x, y, ascii[i], color);
    ...
    int ST7789V::DrawCode(FontxFile *fx, uint16_t x, uint16_t y, uint8_t code, uint16_t color)
    {
        ...
        if (dev._font_direction == 0)
            x = DrawChar(fx, x, y, code, color);
        if (dev._font_direction == 1)
            y = DrawChar(fx, x, y, code, color);
        if (dev._font_direction == 2)
            x = DrawChar(fx, x, y, code, color);
        if (dev._font_direction == 3)
            y = DrawChar(fx, x, y, code, color);
...
int DrawUTF8Char(TFT_t * dev, FontxFile *fx, uint16_t x,uint16_t y,uint8_t *utf8,uint16_t color) {
	...
	return DrawSJISChar(fx, x, y, sjis[0], color);
}
...
int DrawUTF8String(TFT_t * dev, FontxFile *fx, uint16_t x, uint16_t y, unsigned char *utfs, uint16_t color) {
	...
	for(i=0;i<spos;i++) {
		...
		if (dev._font_direction == 0)
			x = DrawSJISChar(fx, x, y, sjis[i], color);
		if (dev._font_direction == 1)
			y = DrawSJISChar(fx, x, y, sjis[i], color);
		if (dev._font_direction == 2)
			x = DrawSJISChar(fx, x, y, sjis[i], color);
		if (dev._font_direction == 3)
			y = DrawSJISChar(fx, x, y, sjis[i], color);
    ...
    // Set font direction
    // dir:Direction
    void ST7789V::SetFontDirection(uint16_t dir)
    {
    ...
    void ST7789V::UnsetFontUnderLine()
    {
        dev._font_underline = false;
    }

    // Backlight OFF
    void ST7789V::BacklightOff()
    {
        if (dev._bl >= 0)
        {
    ...
    void ST7789V::InversionOff()
    {
        spi_master_write_command(0x20); // Display Inversion Off
    }

    // Display Inversion On
    void ST7789V::InversionOn()
    {
        spi_master_write_command(0x21); // Display Inversion On
    }

    void ST7789V::WrapArround(SCROLL_TYPE_t scroll, int start, int end)
    {
    ...
    // y2:End Y coordinate
    // save:Save buffer
    void ST7789V::InversionArea(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t *save)
    {
        if (x1 >= dev._width)
            return;
        if (x2 >= dev._width)
    ...
    // y2:End Y coordinate
    // save:Save buffer
    void ST7789V::GetRect(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t *save)
    {
        if (x1 >= dev._width)
            return;
        if (x2 >= dev._width)
    ...
    // y2:End Y coordinate
    // save:Save buffer
    void ST7789V::SetRect(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t *save)
    {
        if (x1 >= dev._width)
            return;
        if (x2 >= dev._width)
    ...
    // Draw Frame Buffer
    void ST7789V::DrawFinish()
    {
        if (dev._use_frame_buffer == false)
            return;

        spi_master_write_command(0x2A); // set column(x) address
        spi_master_write_addr(dev._offsetx, dev._offsetx + dev._width - 1);
        spi_master_write_command(0x2B); // set Page(y) address
        spi_master_write_addr(dev._offsety, dev._offsety + dev._height - 1);
        spi_master_write_command(0x2C); // Memory Write
