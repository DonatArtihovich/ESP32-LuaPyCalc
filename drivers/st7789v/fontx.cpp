...
namespace Fontx
{
    ...
    void Font::AddFontx(FontxFile *fx, const char *path)
    {
    ...
    // フォント構造体を初期化
    void Font::InitFontx(FontxFile *fxs, const char *f0, const char *f1)
    {
        AddFontx(&fxs[0], f0);
        AddFontx(&fxs[1], f1);
    }
    ...
    // Open font file
    // フォントファイルをOPEN
    bool Font::OpenFontx(FontxFile *fx)
    {
        FILE *f;
        if (!fx->opened)
        {
    ...
    // Close font file
    // フォントファイルをCLOSE
    void Font::CloseFontx(FontxFile *fx)
    {
        if (fx->opened)
        {
            fclose(fx->file);
    ...
    // フォント構造体の表示
    void Font::DumpFontx(FontxFile *fxs)
    {
        for (int i = 0; i < 2; i++)
        {
            printf("fxs[%d]->path=%s\n", i, fxs[i].path);
    ...
    uint8_t Font::getFontWidth(FontxFile *fx)
    {
        uint8_t _width = 0;
        if (fx->valid)
        {
    ...
    uint8_t Font::getFontHeight(FontxFile *fx)
    {
        uint8_t _height = 0;
        if (fx->valid)
        {
    ...
    */
    ...
    bool Font::GetFontx(FontxFile *fxs, uint8_t ascii, uint8_t *pw, uint8_t *ph)
    {
        int i;
        uint32_t offset;
    ...
    */
    ...
    void Font::Font2Bitmap(uint8_t *fonts, uint8_t *line, uint8_t w, uint8_t h, uint8_t inverse)
    {
        int x, y;
        for (y = 0; y < (h / 8); y++)
        {
    ...
    // Bitmapにアンダーラインを追加
    void Font::UnderlineBitmap(uint8_t *line, uint8_t w, uint8_t h)
    {
        int x, y;
        uint8_t wk;
        for (y = 0; y < (h / 8); y++)
    ...
    // Invert bitmap
    // Bitmapを反転
    void Font::ReversBitmap(uint8_t *line, uint8_t w, uint8_t h)
    {
        int x, y;
        uint8_t wk;
        for (y = 0; y < (h / 8); y++)
    ...
    // フォントパターンの表示
    void Font::ShowFont(uint8_t *fonts, uint8_t pw, uint8_t ph)
    {
        int x, y, fpos;
    ...
    // Displaying Bitmap
    // Bitmapの表示
    void Font::ShowBitmap(uint8_t *bitmap, uint8_t pw, uint8_t ph)
    {
        int x, y, fpos;
    ...
    // Invert 8-bit data
    // 8ビットデータを反転
    uint8_t Font::RotateByte(uint8_t ch1)
    {
        uint8_t ch2 = 0;
        int j;
        for (j = 0; j < 8; j++)
